...
import { Mic, Loader2, Phone, ArrowLeft } from 'lucide-react';
...
export default function VoiceSignUpPage() {
  ...
  const [role, setRole] = useState<'elder' | 'nok'>('elder');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/signup/verify?phone=${encodeURIComponent(formattedPhone)}&name=${encodeURIComponent(name)}&role=${role}&method=voice`);
    } catch (err: any) {
        ...
        <Card className="bg-white/90 backdrop-blur-sm rounded-[24px] shadow-lg p-8 md:p-12 w-full">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-[#F4A89F] rounded-full flex items-center justify-center">
                <Mic className="w-6 h-6 text-white" strokeWidth={1.5} />
              </div>
              <div>
                <h2 className="text-sm font-semibold text-deep-navy">Voice Registration</h2>
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl md:text-4xl font-bold text-deep-navy mb-2">Let's Get Started</h1>
            <p className="text-body text-deep-navy/70">
              We'll set up your voice profile after verification
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <Label className="text-deep-navy font-semibold mb-3 block">I am a...</Label>
              <RadioGroup value={role} onValueChange={(value) => setRole(value as 'elder' | 'nok')}>
                <div className="flex items-center space-x-3 bg-soft-gray/20 rounded-[12px] p-4 mb-3">
                  ...
                  <Label htmlFor="elder" className="cursor-pointer flex-1">
                    <span className="font-semibold text-deep-navy">For Myself</span>
                    <p className="text-sm text-deep-navy/60">I will use Grace Companion for my own care</p>
                  </Label>
                </div>
                <div className="flex items-center space-x-3 bg-soft-gray/20 rounded-[12px] p-4">
                  ...
                  <Label htmlFor="nok" className="cursor-pointer flex-1">
                    <span className="font-semibold text-deep-navy">Family Member / Caregiver</span>
                    <p className="text-sm text-deep-navy/60">I will help manage care for a loved one</p>
                  </Label>
                </div>
              </RadioGroup>
            </div>
            ...
            <Button
              type="submit"
              disabled={loading}
              className="w-full h-14 text-xl font-semibold rounded-[16px] bg-[#F4A89F] hover:bg-[#F4A89F]/90 text-white shadow-lg"
            >
              {loading ? (
                <>
