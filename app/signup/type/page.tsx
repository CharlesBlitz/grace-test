...
import { User, Loader2, Phone, ArrowLeft } from 'lucide-react';
import Link from 'next/link';
import { useAuth } from '@/lib/authContext';
import HomeNav from '@/components/HomeNav';

export default function TypeSignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/signup/verify?phone=${encodeURIComponent(formattedPhone)}&name=${encodeURIComponent(name)}&role=${role}&method=type`);
    } catch (err: any) {
        ...
        <Card className="bg-white/90 backdrop-blur-sm rounded-[24px] shadow-lg p-8 md:p-12 w-full">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-[#A8D5BA] rounded-full flex items-center justify-center">
                <User className="w-6 h-6 text-white" strokeWidth={1.5} />
              </div>
              <div>
                <h2 className="text-sm font-semibold text-deep-navy">Type Registration</h2>
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl md:text-4xl font-bold text-deep-navy mb-2">Create Your Account</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Button
              type="submit"
              disabled={loading}
              className="w-full h-14 text-xl font-semibold rounded-[16px] bg-[#A8D5BA] hover:bg-[#A8D5BA]/90 text-white shadow-lg"
            >
              {loading ? (
                <>
